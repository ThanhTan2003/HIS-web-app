...
import { faSearch, faVialVirus, faXmark, faNotesMedical } from '@fortawesome/free-solid-svg-icons';
...
const ThemDichVuNgoaiChuyenKhoa = ({ isOpen, onClose, onSuccess }) => {
    ...
    const getServiceTypes = async (accessToken) => {
        try {
            const response = await fetch(
                `${CONFIG.API_GATEWAY}/medical/service-type/get-all?page=1&size=100`,
                {
                    method: "GET",
                    headers: {
    ...
    const getServices = async (accessToken) => {
        try {
            const response = await fetch(
                `${CONFIG.API_GATEWAY}/medical/service/service-type/not-null?doctor-id=${doctorId}&service-type-id=${serviceTypeId}&keyword=${keyword}&page=${currentPage}&size=${pageSize}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
    ...
    useEffect(() => {
      ...
      console.log(`${CONFIG.API_GATEWAY}/medical/service/service-type/not-null?doctor-id=${doctorId}&service-type-id=${serviceTypeId}&keyword=${keyword}&page=${currentPage}&size=${pageSize}`)
        const accessToken = getToken();
        if (!accessToken) {
            navigate("/login");
        } else {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50 backdrop-blur-none transition-opacity duration-300 ease-in-out opacity-100">
            ...
            <div className="bg-white rounded-lg shadow-lg w-1/2 max-h-[95vh] overflow-y-auto">
                <div className="bg-sky-600 text-white p-2 px-6 rounded-t-lg flex justify-between items-center sticky top-0 z-10">
                    <h2 className="text-lg font-bold"><FontAwesomeIcon icon={faVialVirus} /> &nbsp; Thêm dịch vụ ngoài chuyên khoa</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="mb-4 p-6">
                    <div className="mb-4">
                        <label className="block text-gray-700 font-bold mb-2">Chọn loại dịch vụ:</label>
                        <select
                            className="border rounded p-2 w-full border-blue-300"
                            value={selectServiceType}
                            onChange={(e) => {
                                setSelectServiceType(e.target.value)
                        ...
                        >
                            {
                                serviceTypes.length === 0 ? (
                                    // Nếu không có
                                    <option key={0} value="">Không có dữ liệu</option>
                                ) : serviceTypes.length === 1 ? (
                                    // Nếu chỉ có 1
                                    <option key={serviceTypes[0].id} value={serviceTypes[0].id}>{serviceTypes[0].name}</option>
                                ) : (
                                    // Nếu có nhiều
                                    <>
                                        <option value="" key="">Tất cả</option>
...
export default ThemDichVuNgoaiChuyenKhoa;
